...
function recargaImg(img, mod) {
    $('#foto').html('<img class="thumb" src="thumb/phpThumb.php?src=../modulo/' + mod + '/uploads/files/thumbnail/' + img + '&amp;w=120&amp;h=75&amp;far=1&amp;bg=FFFFFF&amp;hash=361c2f150d825e79283a1dcc44502a76" alt="">');
}

function recargaImgU(img, mod) {
    $('#fotoU').html('<img class="thumb" src="thumb/phpThumb.php?src=../modulo/' + mod + '/uploads/files/thumbnail/' + img + '&amp;w=120&amp;h=75&amp;far=1&amp;bg=FFFFFF&amp;hash=361c2f150d825e79283a1dcc44502a76" alt="">');
}
...
function adicFila(idForm, p) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            fac = $('input:radio[name=fac]:checked').val();

            if (fac == 'on') {
                iva = 0.13;
            }

            iva = (parseFloat(subPrecio) - parseFloat(des) - parseFloat(bon)) * iva;
            $('#tabla tfoot').find('tr').eq(3).find('th').eq(1).find('input').val(iva.toFixed(2));

            total = parseFloat(subPrecio) - parseFloat(des) - parseFloat(bon) + parseFloat(iva);
            $('#tabla tfoot').find('tr').eq(4).find('th').eq(1).find('input').val(total.toFixed(2));
