 ...
 */
    ...
    function getFotos(){
        global $db;
        global $idEmp;
        $strQuery = "SELECT foto FROM empleado WHERE id_empleado = '".$idEmp."' ";
        $strQ = $db->Execute($strQuery);
        $array = array();
        while( $row = $strQ->FetchRow() ){
            array_push($array, $row['foto']);
        }
        return $array;
    }
    ...
    function deleteFotos($foto){
        /*global $db;
        $strQuery = "DELETE FROM files WHERE name = '".$foto."'";
        $strQ = $db->Execute($strQuery);*/
    }

    function insertFotos($foto, $size){
        global $db;
        $strQuery = "INSERT INTO auxImgEmp (name, size) VALUE ('".$foto."', '".$size."')";
        $strQ = $db->Execute($strQuery);
    }
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    insertFotos($file->name, $file->size);
                }
            } else {
