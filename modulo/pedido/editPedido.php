...
<form id="formPreVenta" class="form-horizontal" action="javascript:savePedido('formPreVenta','update.php')" >
	...
	<div class="row">
		<div id="preventa" class="col-xs-12 col-sm-12 col-md-12">
			...
			<div id="preder" class="col-md-6">
				<div class="form-group">
					<label for="pedido" class="col-sm-offset-4 col-sm-4 control-label">N&deg; Pedido: </label>
					<div class="col-sm-4">
				...
				<div class="form-group">
					<label for="factura" class="col-sm-offset-4 col-sm-4 control-label">N&deg; Factura: </label>
					<div class="col-sm-4">
						<input id="factura" name="factura" type="text" disabled value="<?=$file['numFac'];?>" class="form-control"/>
					</div>
				</div>
			</div><!--End preder-->
		</div><!--End preventa-->
	...
	<div class="row" style="border-bottom: 2px #01406b groove;">
	    	...
	    	<table id="tabla" align="center">
	    			...
	    			while ($row = $str->FetchRow()) {
						...
						?>
	    						<td class="det"><?=$row['id_inventario']?> <?=$row['detalle']?>
	    							<input type="hidden" id="item" name="item" value="<?=$row['id_inventario']?>" >
	    						</td>
    							...
    							<td>
    								<input type="text" disabled="disabled" id="precio" name="precio" value="<?=$row['precio']?>" >
    								<input type="hidden" id="precio" name="precio" value="<?=$row['precio']?>" >
	    						</td>

								<td></td>
								<td></td>
								<td>
									<input type="text" disabled="disabled" id="subTotal" name="subTotal" value="<?=number_format($row['cantidad'] * $row['precio'], 2, '.', '')?>" >
								</td>
								<td align="right" class="delete">
									<a class="del" onclick="eliminarFila('<?=$row['id_pedido']?>', '<?=$row['cantidad']?>', '<?=$row['id_inventario']?>' )"><i class="fa fa-ban fa-2x" aria-hidden="true"></i></a>
								</td>
	    					</tr>
					<?PHP
					}
				...
				<tfoot>
					...
					<tr>
						<th colspan="5">IVA:</th>
						<th>
							<input type="text" id="iva" name="iva" autocomplete="off" onKeyUp="calculaDes();" > Bs
						</th>
						<th></th>
					</tr>
					<tr>
		...
		<div id="ventDer" class="col-md-2">
			<div id="ventDe" class="col-md-12" style="padding-right: 0; padding-left: 0">
			...
			<div id="ventDe" class="col-md-12" style="padding-right: 0; padding-left: 0">
				...
				if ($file['fac'] == 'on') {
				?>
					<label class="control-label"><input type="radio" value="on" name="fac" id="fac" checked ><span>&nbsp;</span> Con Factura</label>
					<label class="control-label"><input type="radio" value="off" name="fac" id="fac" data-validation="required" data-validation-error-msg-container="#error-container-radio-fac" ><span>&nbsp;</span> Sin Factura</label>
					<br>
				<?PHP
				} else {
				?>
					<label class="control-label"><input type="radio" value="on" name="fac" id="fac" ><span>&nbsp;</span> Con Factura</label>
					<label class="control-label"><input type="radio" value="off" name="fac" id="fac" checked data-validation="required" data-validation-error-msg-container="#error-container-radio-fac" ><span>&nbsp;</span> Sin Factura</label>
					<br>
				<?PHP
				}
				?>
...
$(document).ready(function() {
	...
	$('input:radio[name=fac]').on('ifChecked', function(event){
		recargaFila();
	});
